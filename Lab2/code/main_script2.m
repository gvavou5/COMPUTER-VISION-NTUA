...
save_pat1 = './results-handrec/MS%d.jpg';
save_pat2 = './results-opticalflow/MS%d-%d.jpg';
save_pat3 = './results-energy/MS%d-%d.jpg';

%diavasma olwn twn eikonwn se rgb kai grayscale kai twn maskwn
for i=1:61
    Irgb{i} = im2double(imread(sprintf(file_pat,i)));
	I{i} = im2double(rgb2gray(imread(sprintf(file_pat,i))));
	U{i} = im2double(imread(sprintf(file_pat2,i)));
end
...
for i=1:60
	...
	[dx, dy] = lk_ms(I1_w, I2_w, 8.9 , 0.008 , 4);
	...
	Energy = dx.^2+dy.^2;												% ypologismos energeias
    	fig_ener = figure(7);
    	imshow(Energy, []);													% kai apeikonisi ths
    	print(fig_ener, sprintf(save_pat3, i, i+1), '-djpeg');
	
	[displ_x,displ_y] = displ(dx,dy);    %eyresh metatopishs bounding box
    	x = x - displ_x;             	     %ananewsh dianusmatos 8eshs xeriou
    	y = y - displ_y;
